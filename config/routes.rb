Rails.application.routes.draw do
  ...
  resources :users do
    resources :items, only: [:new, :create, :edit, :update]#<----- this is for the lenders/host
  end

  namespace :s do#<----- search
    resources :items, only: [:index]#<------ search results
  end
  ...
  resources :items, only: [:show]# <----- /equipments/:equipment_id/show ; this shows the details about the equipment
